...
namespace ImageHeaven
{
    public partial class frmCR : Form
    {
        ...
        public frmCR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, eSTATES[] prmState)
        {
            InitializeComponent();
        ...
        public frmCR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file, string stage)
        {
            InitializeComponent();
        ...
        public frmCR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file)
        {
            InitializeComponent();
        ...
        public DataTable _GetFileCaseDetailsIndividual(string proj, string bundle, string fileName)
        {
            ...
            string sql = "select distinct proj_code, bundle_Key,item_no,filename,ps_name,ps_code,div_name,div_code," +
                         "date_format(CR_date, '%Y-%m-%d'),CR_case_no " +
        ...
        private void frmCR_Load(object sender, EventArgs e)
        {
            ...
            if (_mode == Mode._Edit)
            {
                string CRDate = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][8].ToString();
                string CRcaseno = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][9].ToString();
                deTextBox12.Enabled = false;

                if (CRDate != "")
                {
                    deTextBox10.Text = CRDate.Substring(8, 2);
                    deTextBox11.Text = CRDate.Substring(5, 2);
                    deTextBox12.Text = CRDate.Substring(0, 4);

                }

                deTextBox9.Text = CRcaseno;
        ...
        public bool validate()
        {
            ...
            if (deTextBox9.Text == "" || deTextBox9.Text == null || String.IsNullOrEmpty(deTextBox9.Text) || String.IsNullOrWhiteSpace(deTextBox9.Text))
            {
                ...
                MessageBox.Show("You cannot leave CR Case Number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox9.Focus();
                return retval;
            }
            else
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please input valid numeric CR Case Number...", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    deTextBox9.Focus();
                    return retval;
                }
            }
            ...
            if ((isSDate != ""))
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please select a valid CR date", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private bool checkFileNotExists(string proj, string bundle, string file)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                MessageBox.Show("This CR file number already exists for this batch", "B'Zer - Confirmation !", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                retval = false;
                deTextBox10.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string psname, string pscode, string divname, string divcode, string filename, string CRcaseno, string CRdate, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,CR_date,CR_case_no,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + CRdate + "','" + CRcaseno + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            }
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,CR_date,CR_case_no,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + CRdate + "','" + CRcaseno + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
            }
        ...
        public bool updateMetaEdit(string divN, string divC, string psN, string psC, string CRdate, string CRcaseno)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, divN, divC, psN, psC, CRdate, CRcaseno);
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string divN, string divC, string psN, string psC, string cRDate, string cRcaseno)
        {
            string sqlStr = null;
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '" + fileName + "',ps_name  = '" + psN + "',ps_code = '" + psC + "'," +
                "div_name = '" + divN + "',div_code = '" + divC + "',CR_date= '" + cRDate + "',CR_case_no='" + cRcaseno + "'," +
            ...
            if (_mode == Mode._Add)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (validate() == true)
                    {
                        ...
                        string CRcaseNumber = deTextBox9.Text.Trim();

                        if (deTextBox12.Text != "" && deTextBox11.Text != "" && deTextBox10.Text != "")
                        {
                            CRDate = deTextBox12.Text + "-" + deTextBox11.Text + "-" + deTextBox10.Text;
                        }
                        else
                        {
                            CRDate = "";
                        ...
                        string filenumber = "CR" + CRcaseNumber + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (checkFileNotExists(projKey, bundleKey, filename) == true)
                        {
                            ...
                            bool insertCase = insertIntoMeta(item, psname, pscode, divname, divcode, filename, CRcaseNumber, CRDate, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
            ...
            if (_mode == Mode._Edit)
            {
                if (currStage == "Entry")
                {
                    if (validate() == true)
                    {
                        ...
                        string CRcaseno = deTextBox9.Text.Trim();

                        string filenumber = "CR" + CRcaseno.Trim() + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (filenumber.Trim() != Files.filename)
                        {
                            if (checkFileNotExistsEdit(filenumber.Trim(), projKey, bundleKey) == true)
                            {
                                ...
                                bool updateMeta = updateMetaEdit(divname, divcode, psname, pscode, CRDate, CRcaseno);
        ...
        private void deTextBox9_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                if (_mode == Mode._Add)
                {
                    string filenumber = "CR" + deTextBox9.Text.Trim() + deTextBox12.Text;
                    checkFileNotExists(projKey, bundleKey, filenumber.Trim());
                }
            }
        }

        private void frmCR_KeyUp(object sender, KeyEventArgs e)
        {
