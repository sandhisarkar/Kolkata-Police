...
namespace ImageHeaven
{
    public partial class frmMR : Form
    {
        ...
        public frmMR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, eSTATES[] prmState)
        {
            InitializeComponent();
        ...
        public frmMR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file, string stage)
        {
            InitializeComponent();
        ...
        public frmMR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file)
        {
            InitializeComponent();
        ...
        public DataTable _GetFileCaseDetailsIndividual(string proj, string bundle, string fileName)
        {
            ...
            string sql = "select distinct proj_code, bundle_Key,item_no,filename,ps_name,ps_code,div_name,div_code," +
                         "date_format(MR_date, '%Y-%m-%d'),MR_case_no,MR_serial_no " +
        ...
        private void frmMR_Load(object sender, EventArgs e)
        {
            string month_year = _GetBundleDetails().Rows[0][8].ToString();
            if (_mode == Mode._Add)
            {
            ...
            if (_mode == Mode._Edit)
            {
                string MRDate = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][8].ToString();
                string MRcaseno = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][9].ToString();
                string MRserialno = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][10].ToString();

                deTextBox12.Enabled = false;

                if (MRDate != "")
                {
                    deTextBox10.Text = MRDate.Substring(8, 2);
                    deTextBox11.Text = MRDate.Substring(5, 2);
                    deTextBox12.Text = MRDate.Substring(0, 4);

                }

                deTextBox9.Text = MRserialno;
                deTextBox6.Text = MRcaseno;
        ...
        public bool validate()
        {
            ...
            if (deTextBox9.Text == "" || deTextBox9.Text == null || String.IsNullOrEmpty(deTextBox9.Text) || String.IsNullOrWhiteSpace(deTextBox9.Text))
            {
                ...
                MessageBox.Show("You cannot leave MR Serial Number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox9.Focus();
                return retval;
            }
            else
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please input valid numeric MR Serial Number...", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    deTextBox9.Focus();
                    return retval;
                }
            }

            if (deTextBox6.Text == "" || deTextBox6.Text == null || String.IsNullOrEmpty(deTextBox6.Text) || String.IsNullOrWhiteSpace(deTextBox6.Text))
            {
                retval = false;

                MessageBox.Show("You cannot leave MR Case Number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox9.Focus();
                return retval;
            }
            else
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please input valid numeric MR Case Number...", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    deTextBox6.Focus();
                    return retval;
                }
            }
            ...
            if ((isSDate != ""))
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please select a valid MR date", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private bool checkFileNotExists(string proj, string bundle, string file)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                MessageBox.Show("This MR file number already exists for this batch", "B'Zer - Confirmation !", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                retval = false;
                deTextBox10.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string psname, string pscode, string divname, string divcode, string filename, string MRserial, string MRdate, string MRCase, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,MR_date,MR_serial_no,MR_case_no,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + MRdate + "','" + MRserial + "','" + MRCase + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            }
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,MR_date,MR_serial_no,MR_case_no,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + MRdate + "','" + MRserial + "','" + MRCase + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
            }
        ...
        public bool updateMetaEdit(string divN, string divC, string psN, string psC, string MRSerialno, string MRdate, string MRcaseno)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, divN, divC, psN, psC, MRSerialno, MRdate, MRcaseno);
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string divN, string divC, string psN, string psC, string mRSerialno, string mRDate, string mRcaseno)
        {
            string sqlStr = null;
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '" + fileName + "',ps_name  = '" + psN + "',ps_code = '" + psC + "'," +
                "div_name = '" + divN + "',div_code = '" + divC + "',MR_serial_no = '" + mRSerialno + "' ,MR_date= '" + mRDate + "',MR_case_no='" + mRcaseno + "'," +
            ...
            if (_mode == Mode._Add)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (validate() == true)
                    {
                        ...
                        string MRserialNumber = deTextBox9.Text.Trim();
                        string MRcaseNumber = deTextBox6.Text.Trim();


                        if (deTextBox12.Text != "" && deTextBox11.Text != "" && deTextBox10.Text != "")
                        {
                            MRDate = deTextBox12.Text + "-" + deTextBox11.Text + "-" + deTextBox10.Text;
                        }
                        else
                        {
                            MRDate = "";
                        ...
                        string filenumber = "MR" + MRserialNumber + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (checkFileNotExists(projKey, bundleKey, filename) == true)
                        {
                            ...
                            bool insertCase = insertIntoMeta(item, psname, pscode, divname, divcode, filename, MRserialNumber, MRDate, MRcaseNumber, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
            ...
            if (_mode == Mode._Edit)
            {
                if (currStage == "Entry")
                {
                    if (validate() == true)
                    {
                        ...
                        string MRserialno = deTextBox9.Text.Trim();
                        string MRcaseno = deTextBox6.Text.Trim();

                        string filenumber = "MR" + MRserialno + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (filenumber.Trim() != Files.filename)
                        {
                            if (checkFileNotExistsEdit(filenumber.Trim(), projKey, bundleKey) == true)
                            {
                                ...
                                bool updateMeta = updateMetaEdit(divname, divcode, psname, pscode, MRserialno, MRDate, MRcaseno);
        ...
        private void frmMR_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void deTextBox6_Leave(object sender, EventArgs e)
        {
            if (deTextBox6.Text == "" || deTextBox6.Text == null || String.IsNullOrEmpty(deTextBox6.Text) || String.IsNullOrWhiteSpace(deTextBox6.Text))
            { return; }
            else
