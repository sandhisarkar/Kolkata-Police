...
namespace ImageHeaven
{
    public partial class frmFIR : Form
    {
        ...
        public frmFIR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, eSTATES[] prmState)
        {
            InitializeComponent();
        ...
        public frmFIR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file, string stage)
        {
            InitializeComponent();
        ...
        public frmFIR(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file)
        {
            InitializeComponent();
        ...
        public DataTable _GetFileCaseDetailsIndividual(string proj, string bundle, string fileName)
        {
            ...
            string sql = "select distinct proj_code, bundle_Key,item_no,filename,ps_name,ps_code,div_name,div_code," +
                         "date_format(GD_serial_date, '%Y-%m-%d'),FIR_caseno " +
        ...
        private void frmFIR_Load(object sender, EventArgs e)
        {
            string month_year = _GetBundleDetails().Rows[0][8].ToString();
            if (_mode == Mode._Add)
            {
            ...
            if (_mode == Mode._Edit)
            {
                string gdserialDate = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][8].ToString();
                string fircaseno = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][9].ToString();
                deTextBox12.Enabled = false;
                
                if (gdserialDate != "")
                {
                    deTextBox10.Text = gdserialDate.Substring(8, 2);
                    deTextBox11.Text = gdserialDate.Substring(5, 2);
                    deTextBox12.Text = gdserialDate.Substring(0, 4);

                }

                deTextBox9.Text = fircaseno;
        ...
        public bool validate()
        {
            ...
            if (deTextBox9.Text == "" || deTextBox9.Text == null || String.IsNullOrEmpty(deTextBox9.Text) || String.IsNullOrWhiteSpace(deTextBox9.Text))
            {
                ...
                MessageBox.Show("You cannot leave Case Number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox9.Focus();
                return retval;
            }
            else
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please input valid numeric Case Number...", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    deTextBox9.Focus();
                    return retval;
                }
            }
            ...
            if ((isSDate != ""))
            {
                ...
                {
                    retval = false;
                    MessageBox.Show(this, "Please select a valid GD serial number date", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private bool checkFileNotExists(string proj, string bundle, string file)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                MessageBox.Show("This FIR file number already exists for this batch", "B'Zer - Confirmation !", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                retval = false;
                deTextBox10.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string psname, string pscode, string divname, string divcode, string filename, string FIRcaseno, string gdserialnodate, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            if (frmFIR.state[0] == eSTATES.METADATA_ENTRY)
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,gd_serial_date,FIR_caseno,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + gdserialnodate + "','" + FIRcaseno + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            ...
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,bundle_key,item_no,ps_name,ps_code,div_name,div_code,filename,gd_serial_date,FIR_caseno,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + psname + "','" + pscode + "','" + divname + "','" + divcode + "','" + filename + "','" + gdserialnodate + "','" + FIRcaseno + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
                //sqlCmd.Connection = sqlCon;
            }
            sqlCmd.Connection = sqlCon;
        ...
        public bool updateMetaEdit(string divN, string divC, string psN, string psC, string gsserialdate, string fircaseno)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, divN, divC, psN, psC, gsserialdate, fircaseno);
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string divN, string divC, string psN, string psC, string gdSerialDate, string Fircaseno)
        {
            string sqlStr = null;
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '" + fileName + "',ps_name  = '" + psN + "',ps_code = '" + psC + "'," +
                "div_name = '" + divN + "',div_code = '" + divC + "',gd_serial_date= '" + gdSerialDate + "',fir_caseno='" + Fircaseno + "'," +
        ...
        private void deButtonSave_Click(object sender, EventArgs e)
        {
            ...
            if (_mode == Mode._Add)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (validate() == true)
                    {
                        string gdSerialDate = string.Empty;
                        ...
                        string caseNumber = deTextBox9.Text.Trim();

                        if (deTextBox12.Text != "" && deTextBox11.Text != "" && deTextBox10.Text != "")
                        {
                            gdSerialDate = deTextBox12.Text + "-" + deTextBox11.Text + "-" + deTextBox10.Text;
                        }
                        else
                        {
                            gdSerialDate = "";
                        ...
                        string filenumber = "FIR" + caseNumber + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (checkFileNotExists(projKey, bundleKey, filename) == true)
                        {
                            ...
                            bool insertCase = insertIntoMeta(item, psname, pscode, divname, divcode, filename, caseNumber, gdSerialDate, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
            ...
            if (_mode == Mode._Edit)
            {
                if (currStage == "Entry")
                {
                    if (validate() == true)
                    {
                        ...
                        string fircaseno = deTextBox9.Text.Trim();

                        string filenumber = "FIR" + fircaseno.Trim() + deTextBox12.Text;
                        filename = filenumber;
                        ...
                        if (filenumber.Trim() != Files.filename)
                        {
                            if (checkFileNotExistsEdit(filenumber.Trim(), projKey, bundleKey) == true)
                            {
                                ...
                                bool updateMeta = updateMetaEdit(divname, divcode, psname, pscode, gdserialDate, fircaseno);
        ...
        private void deTextBox9_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                if (_mode == Mode._Add)
                {
                    string filenumber = "FIR" + deTextBox9.Text.Trim() + deTextBox12.Text;
                    checkFileNotExists(projKey, bundleKey, filenumber.Trim());
                }
            }
        }

        private void frmFIR_KeyUp(object sender, KeyEventArgs e)
        {
